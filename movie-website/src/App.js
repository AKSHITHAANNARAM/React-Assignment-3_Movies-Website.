...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isSearchPerformed: false, // Track if a search has been performed
    };
    // Initialize debouncedSearch in the constructor
  ...
  handleSearch = (query) => {
    if (query.trim() === '') {
      // If the query is empty, reset movies to defaultMovies
      this.fetchDefaultMovies();
      this.setState({ isSearchPerformed: false }); // No search has been performed
    } else {
      fetch(`https://www.omdbapi.com/?apikey=45f0782a&s=${query}`)
        .then((response) => response.json())
        .then((data) => {
          if (data.Search) {
            this.setState({ movies: data.Search, isSearchPerformed: true });
          } else {
            this.setState({ movies: [], isSearchPerformed: true });
          }
        })
        .catch((error) => {
  ...
  render() {
    const { movies, query, movieQuotes, isSearchPerformed } = this.state;
    ...
    return (
      <div className="App">
        ...
        <div className="movie-list">
          {isSearchPerformed && movies.length === 0 ? (
